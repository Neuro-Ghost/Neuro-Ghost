[![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=Neuro-Ghost&theme=react-dark)](https://github.com/ashutosh00710/github-readme-activity-graph)
# 👋 Hey, I'm Ahmad Faour

🚀 **Aspiring React Developer | AI Automation Enthusiast | Machine Learning Engineer **  
📍 Based somewhere on the internet, always learning & building.

---

## ⚡ About Me
- 💻 Learning **React**, **Next.js**, and **AI automation tools**  
- 📞 Working in a call-center while transitioning to full-time tech  
- 🎯 Building real projects to level up my portfolio  
- 🧠 Love learning: AI agents, automation, POS systems, and ML basics

---

## 🔧 Tech Stack
**Frontend:** React, JavaScript, HTML, CSS  
**Backend:** Node.js (learning), Python  
**Tools:** Git, GitHub, VS Code, Postman  
**AI:** LangChain, automation scripts, OpenAI API  

---

## 🛠 Projects
Here are some of the projects I'm working on:

### 🔹 POS System (React)
A simple Point of Sale system with item selection, payments, and invoice download.

### 🔹 AI Customer Support Agent
Automates replies, ticket creation, and long-context handling.

### 🔹 RFM Customer Segmentation (Machine Learning)
Built in Google Colab using the UK Online Retail dataset.

---

## 📊 GitHub Stats
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Neuro-Ghost&show_icons=true&theme=default)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Neuro-Ghost&layout=compact)

---

## 📫 Contact
📩 **Email:** a.hfaour@hotmail.com
💼 **LinkedIn:** [click here](https://www.linkedin.com/in/ahmad--faour/) 



